...
|       **\\c**        | **noremap**  |                  | 开启/关闭拼写检查            |                         **:set spell!\<CR\>**                          |
|       **\\s**        | **noremap**  |                  | 全局查找替换                 |                  **:%s///g\<left\>\<left\>\<left\>**                   |
|       **\\t**        | **noremap**  | `vim-table-mode` | 触发表格                     |                       **:TableModeToggle\<CR\>**                       |
...
|   **\<LEADER\>ag**   | **nnoremap** |      `agit`      | 打开git仓库                  |                            **:Agit\<CR\>**                             |
|   **\<LEADER\>bq**   | **noremap**  |     `buffer`     | 关闭当前`buffer` 不保存      |                              **:q\<CR\>**                              |
|   **\<LEADER\>bQ**   | **noremap**  |     `buffer`     | 关闭所有buffer               |                             **:qa\<CR\>**                              |
|   **\<LEADER\>bs**   | **noremap**  |     `buffer`     | 保存当前`buffer`             |                              **:w\<CR\>**                              |
|   **\<LEADER\>bf**   | **noremap**  |     `buffer`     | `buffer`关闭除了第一个       |                              **\<C-w\>o**                              |
|   **\<LEADER\>cc**   | **noremap**  |    `calendar`    | 显示当前时钟                 |             **:Calendar -view=clock -position=here\<CR\>**             |
|   **\<LEADER\>cr**   | **noremap**  |      `call`      | 运行当前文件                 |                    **:call CompileRunGcc()\<CR\>**                     |
|   **\<LEADER\>cs**   | **noremap**  |      `call`      | 打印选中所属组信息           |                       **:call SynGroup()\<CR\>**                       |
|   **\<LEADER\>dw**   | **noremap**  |                  | 相邻的重复词                 |                    **/\\(\\<\\w\+\\>\\)\\_s\*\\1**                     |
|   **\<LEADER\>ec**   | **noremap**  |      `edit`      | 编译 **init.vim** 文件       |                  **:e ~/.config/nvim/init.vim\<CR\>**                  |
|   **\<LEADER\>ev**   | **noremap**  |      `edit`      | 创建一个 **.nvimrc** 文件    |                          **:e .nvimrc\<CR\>**                          |
...
|   **\<LEADER\>ff**   | **noremap**  |      `fzf`       | 正则表达式查找               |                        **\<silent\> :Rg\<CR\>**                        |
|   **\<LEADER\>fh**   | **noremap**  |      `fzf`       | 历史记录                     |                     **\<silent\> :History\<CR\>**                      |
|   **\<LEADER\>fr**   | **noremap**  |      `fzf`       | 查找替换                     |        **:F  \*\*/\*\<left\>\<left\>\<left\>\<left\>\<left\>**         |
|   **\<LEADER\>pf**   | **noremap**  |     `print`      | 将输入转为字符画输出         |                             **:r !figlet**                             |
|   **\<LEADER\>pp**   | **noremap**  |     `print`      | 打印当前文件路径             |                     **:echo expand('%:p')\<CR\>**                      |
|   **\<LEADER\>vf**   | **noremap**  |     `vista`      | 打开函数列表查找窗口         |                  **:silent! Vista finder coc\<CR\>**                   |
...
|  **\<LEADER\>xgd**   | **nmap**     |      `coc`       | 跳转定义                     |                      **\<Plug\>(coc-definition)**                      |
|  **\<LEADER\>xgD**   | **nmap**     |      `coc`       | 跳转定义右侧打开新的buffer   |               **:tab sp\<CR\>\<Plug\>(coc-definition)**                |
|  **\<LEADER\>xgy**   | **nmap**     |      `coc`       | 跳转类型定义                 |                   **\<Plug\>(coc-type-definition)**                    |
|  **\<LEADER\>xgi**   | **nmap**     |      `coc`       | 跳转实现                     |                    **\<Plug\>(coc-implementation)**                    |
|  **\<LEADER\>xgr**   | **nmap**     |      `coc`       | 跳转引用                     |                      **\<Plug\>(coc-references)**                      |
|  **\<LEADER\>xts**   | **nmap**     |      `coc`       | 翻译                         |                     **\<Plug\>(coc-translator-p)**                     |
|  **\<LEADER\>xrs**   | **nmap**     |      `coc`       | 范围选取                     |                     **\<Plug\>(coc-range-select)**                     |
|   **\<LEADER\>x-**   | **nmap**     |      `coc`       | 诊断错误位置向前             |                   **\<Plug\>(coc-diagnostic-prev)**                    |
|   **\<LEADER\>x=**   | **nmap**     |      `coc`       | 诊断错误位置向后             |                   **\<Plug\>(coc-diagnostic-next)**                    |
|  **\<LEADER\>xrn**   | **nmap**     |      `coc`       | 重命名                       |                        **\<Plug\>(coc-rename)**                        |
...
|  **\<LEADER\>xsp**   | **vmap**     |      `coc`       | 选择代码片段                 |                   **\<Plug\>(coc-snippets-select)**                    |
...
|   **\<LEADER\>a**    | **xmap**     |      `coc`       | 代码操作选中部分             |                 **\<Plug\>(coc-codeaction-selected)**                  |
